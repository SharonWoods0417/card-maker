...
## **P005: PDF卡片排版横向居中问题** 🎨
**发现时间**: 2024-12-30  
**问题描述**: PDF中的卡片元素（图片、文字、音标、拼读块、背面例句）未能完全居中对齐，影响视觉效果和专业性。
...
### 💡 **根本原因分析**
PDF布局系统中仅依靠`textAlign: 'center'`不足以实现完全居中，需要在flexbox布局的多个层级明确设置对齐属性。特别是：
1. 音标元素需要`width: '100%'`才能在其容器内正确居中
2. 背面例句容器需要明确的flex对齐属性和内部文本居中设置
...
sentenceContainer: {
  width: '100%',
  // 缺少 alignSelf, display, flexDirection, alignItems
},
sentenceEnglish: {
  // 缺少 textAlign: 'center' 和 width: '100%'
},
```

### 🔧 **解决方案实施**

#### **具体修复的关键问题 - 2024-12-30**:
...
sentenceContainer: {
  ...
  borderRadius: 6,
  width: '100%',
  // 问题：虽然宽度100%，但缺少内部对齐设置
}
sentenceEnglish: {
  fontSize: 10,
  color: '#374151',
  fontFamily: fontFamilies.bold,
  // 问题：没有textAlign和width设置
}
...
sentenceContainer: {
  ...
  borderRadius: 6,
  width: '100%',
  alignSelf: 'center',        // 🆕 容器在父元素中居中
  display: 'flex',            // 🆕 明确flex布局
  flexDirection: 'column',    // 🆕 垂直排列
  alignItems: 'center',       // 🆕 内部内容居中
}
sentenceEnglish: {
  fontSize: 10,
  color: '#374151',
...
const pdfStyles = StyleSheet.create({
  ...
  ipa: { alignSelf: 'center', width: '100%' },          // 🆕 音标完全居中
  phonicsContainer: { alignSelf: 'center' },
  imageContainer: { alignSelf: 'center' },
  sentenceContainer: { alignSelf: 'center', alignItems: 'center' }, // 🆕 例句完全居中
});
```
...
### 🔗 **相关修改文件**
- `src/components/ExportSection.tsx`: PDF样式系统居中对齐全面优化
